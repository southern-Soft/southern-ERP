...
Popular Pantone Fashion, Home + Interiors TCX colors
"""
...
def seed_pantone_tcx_colors():
    """Seed comprehensive Pantone TCX colors"""
    engine = engines[DatabaseType.SAMPLES]
    
    # Comprehensive Pantone TCX colors with hex codes
    # Format: (color_name, tcx_code, hex_code, rgb_r, rgb_g, rgb_b)
    pantone_colors = [
        # REDS
    ...
    with engine.begin() as conn:
        try:
            logger.info("Seeding Pantone TCX colors...")
            
            # Insert all colors as general colors (available to all buyers)
            inserted = 0
            for color_name, tcx_code, hex_code, rgb_r, rgb_g, rgb_b in pantone_colors:
                try:
                    query = text("""
                        ...
                        ON CONFLICT (color_name, buyer_id) 
                        DO UPDATE SET 
                            tcx_code = EXCLUDED.tcx_code,
                    ...
                    logger.warning(f"Failed to insert color {color_name}: {e}")
                    continue
            
            logger.info(f"✅ Successfully seeded {inserted} Pantone TCX colors")
            
            # Get total count
            result = conn.execute(text("SELECT COUNT(*) FROM color_master WHERE is_general = TRUE"))
            total = result.scalar()
            logger.info(f"✅ Total general colors in database: {total}")
            
        except Exception as e:
            logger.error(f"❌ Failed to seed Pantone TCX colors: {e}")
            raise
